...
export const QuotationPrintTemplate = forwardRef(({ data }, ref) => {
  if (!data) return null;
    ...
    >
      ...
      <div className="flex justify-between items-start mb-10">
        <div>
          ...
          {(branch?.gstin || branch?.gstNumber) && <p className="text-slate-600 mt-1">GSTIN: {branch.gstin || branch.gstNumber}</p>}
        ...
        <div className="text-right">
          ...
          <table className="ml-auto text-sm">
            <tbody>
              <tr>
                <td className="text-slate-500 pr-4 py-1 text-right font-medium">Quotation #:</td>
                <td className="text-slate-800 font-medium">{quotationNumber}</td>
              </tr>
              <tr>
                <td className="text-slate-500 pr-4 py-1 text-right font-medium">Date:</td>
                <td className="text-slate-800">{quotationDate}</td>
              </tr>
              <tr>
                <td className="text-slate-500 pr-4 py-1 text-right font-medium">Validity:</td>
                <td className="text-slate-800">{validity}</td>
              </tr>
              <tr>
              ...
              {priority && priority !== 'Normal' && (
                <tr>
                  <td className="text-slate-500 pr-4 py-1 text-right font-medium">Priority:</td>
                  <td className="text-red-600 font-medium">{priority}</td>
                </tr>
              )}
            </tbody>
          </table>
            ...
            {items.map((item, index) => {
              ...
              return (
                <tr key={index} className="text-slate-800 border-b border-slate-200">
                  ...
                  <td className="py-3 px-3 text-center border-x border-slate-200">{taxPct > 0 ? `${taxPct}%` : '-'}</td>
        ...
        <div className="w-72">
          <table className="w-full text-sm">
            <tbody>
              ...
              {shippingAmount > 0 && (
                <tr>
                  <td className="py-1 text-slate-600 text-right pr-4">Shipping:</td>
                  <td className="py-1 text-slate-800 text-right font-medium">{formatCurrency(shippingAmount)}</td>
                </tr>
              )}
      ...
      <div className="mb-12">
        ...
        {terms && (
          <div>
            ...
            <p className="text-slate-600 text-xs whitespace-pre-wrap leading-relaxed">{terms}</p>
          </div>
        )}
      </div>
